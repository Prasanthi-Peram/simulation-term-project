...
# ------------------- PARAMETERS -------------------
def run_T1_simulation(random_seed=None):
    global RANDOM_SEED
    ...
    results = {
        ...
        "avg_wait_security_dom": avg(waits["security"]), # Assuming all security is domestic for T
...
def run_T2_simulation(random_seed=None):
    global RANDOM_SEED
    RANDOM_SEED = random_seed if random_seed is not None else 42
...
def run_T3_simulation(random_seed=None):
    global RANDOM_SEED
    RANDOM_SEED = random_seed if random_seed is not None else 42
...
def run_multiple_replications(sim_func, n_replications=5, base_seed=42):
    results = []
...
def analyze_results(results_list):
    ...
    n = len(df)
    t_value = t.ppf(0.975, n - 1)  # 95% confidence level

    summary = []
    for metric in df.columns:
        mean_val = df[metric].mean()
        std_val = df[metric].std(ddof=1)
        ci_half_width = t_value * std_val / np.sqrt(n)
        summary.append({
            "Metric": metric,
            "Mean": round(mean_val, 3),
            "Std. Dev": round(std_val, 3),
            "95% Confidence Interval": f"{round(mean_val, 3)} ± {round(ci_half_width, 3)}"
        })
...
def run_analysis(num_replications=10):
    print(f"Running {num_replications} replications for each terminal...\n")

    # ✅ Each terminal gets its own base seed for independence
    all_results_T1 = run_multiple_replications(run_T1_simulation, num_replications, base_seed=42)
    all_results_T2 = run_multiple_replications(run_T2_simulation, num_replications, base_seed=100)
    all_results_T3 = run_multiple_replications(run_T3_simulation, num_replications, base_seed=200)

    print("\n--- ANALYSIS RESULTS ---\n")

    summary_T1 = analyze_results(all_results_T1)
    summary_T2 = analyze_results(all_results_T2)
    summary_T3 = analyze_results(all_results_T3)

    print("\n📊 Terminal 1 Output Summary:")
    print(summary_T1.to_string(index=False))
    print("\n📊 Terminal 2 Output Summary:")
    print(summary_T2.to_string(index=False))
    print("\n📊 Terminal 3 Output Summary:")
    print(summary_T3.to_string(index=False))

    # Optional: Save to Excel for reporting
    with pd.ExcelWriter("Terminal_Output_Analysis.xlsx") as writer:
        summary_T1.to_excel(writer, sheet_name="T1", index=False)
        summary_T2.to_excel(writer, sheet_name="T2", index=False)
        summary_T3.to_excel(writer, sheet_name="T3", index=False)

    print("\n✅ Analysis completed and saved to 'Terminal_Output_Analysis.xlsx'.")
    return summary_T1, summary_T2, summary_T3, all_results_T1, all_results_T2, all_results_T3


# ---------- RUN ----------
if __name__ == "__main__":
    summary_T1, summary_T2, summary_T3, all_results_T1, all_results_T2, all_results_T3 = run_analysis(num_replications=10)

import matplotlib.pyplot as plt

def plot_confidence_intervals(summary_df, terminal_name):
    plt.figure(figsize=(8, 5))
    plt.bar(summary_df["Metric"], summary_df["Mean"], yerr=[float(x.split('±')[1]) for x in summary_df["95% Confidence Interval"]],
            capsize=5)
    plt.title(f"{terminal_name} - Mean with 95% CI")
    plt.ylabel("Mean Value")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.show()

plot_confidence_intervals(summary_T1, "Terminal 1")
plot_confidence_intervals(summary_T2, "Terminal 2")
plot_confidence_intervals(summary_T3, "Terminal 3")

def plot_boxplots(results_list, terminal_name):
    df = pd.DataFrame(results_list)
    df.boxplot(figsize=(8,5))
    plt.title(f"{terminal_name} - Distribution Across Replications")
    plt.ylabel("Value")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.show()

plot_boxplots(all_results_T1, "Terminal 1")
plot_boxplots(all_results_T2, "Terminal 2")
plot_boxplots(all_results_T3, "Terminal 3")