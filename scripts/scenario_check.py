...
def run_T1_simulation(alpha):
    ...
    def passenger(env, pid, airline, gate_reg, gate_digi, checkin_indigo, checkin_spicejet, securities, boardings):
        ...
        throughput[idx] += 1
        if airline in results_by_airline: # Added check for airline
    ...
    def flight_source(env, gate_reg, gate_digi, checkin_indigo, checkin_spicejet, securities, boardings):
        ...
        for _, row in df.iterrows():
            ...
            n_passengers = int(row["N_passengers"]*alpha) # Ensured n_passengers is integer
            airline = row["Airline"]
            ...
            inter_arrivals = np.random.exponential(scale=mean_arrival_gap, size=int(n_passengers)) # Ensured size is integer
    ...
    results = {
        ...
        "avg_wait_checkin": (waits["checkin_indigo"] + waits["checkin_spicejet"]),
...
def run_T2_simulation(alpha):
    ...
    busy_time = {"checkin": 0, "security": 0, "boarding": 0}
    throughput = np.zeros(28)  # for 27 hours
    results_by_airline = {"IndiGo": [], "Akasa Air": []} # Initialize with known airlines
    system_count = []
    ...
    # Security
        ...
        idx = min(int(env.now // 60), len(throughput) - 1)
        throughput[idx] += 1
        # Ensure airline key exists before appending
        if airline not in results_by_airline:
             results_by_airline[airline] = [] # Dynamically add new airline
        results_by_airline[airline].append(env.now - t0)
        system_count.append((env.now, -1))
    ...
    def flight_source(env, gate_reg, gate_digi, checkin_indigo, checkin_akasa_air, securities, boardings):
        ...
        for _, row in df.iterrows():
            dep_time = row["Dep_time_min"]
            n_passengers = int(row["N_passengers"])*alpha # Ensure n_passengers is an integer
            airline = row["Airline"]
            arr_start = max(0.0, dep_time - 120.0)
            arr_end = min(RUN_TIME - eps, dep_time - 60.0)
            ...
            if total_window <= 0:
                continue
            mean_arrival_gap = total_window / n_passengers
            inter_arrivals = np.random.exponential(scale=mean_arrival_gap, size=int(n_passengers)) # Ensure size is integer
            ...
            for arr_time in arrival_times:
                ...
                env.process(
                    passenger(
                        ...
                        gate_reg, gate_digi,
                        checkins, securities_dom, securities_int,
...
# Run simulations
results_1 = run_T1_simulation(alpha=0.80)  # 10% increased demand
results_2 = run_T1_simulation(alpha=1.00)  # baseline

# Extract comparison metrics
metrics = [
    "avg_total_time",
    "avg_wait_checkin",
    "avg_wait_security_dom"
]

labels = {
    "avg_total_time": "Total Time in System",
    "avg_wait_checkin": "Check-in Wait Time",
    "avg_wait_security_dom": "Security Wait Time"
}

baseline = [results_2[m] for m in metrics]
increased = [results_1[m] for m in metrics]

x = np.arange(len(metrics))
width = 0.35

plt.figure(figsize=(10,6))
plt.bar(x - width/2, baseline, width, label="α = 1.0 (Baseline)", color="skyblue")
plt.bar(x + width/2, increased, width, label="α = 0.8 (20% Decreased Load)", color="salmon")

plt.xticks(x, [labels[m] for m in metrics], fontsize=12)
plt.ylabel("Time (minutes)", fontsize=13)
plt.title("Comparison of Passenger Waiting Times Before and After Decreasing Arrivals", fontsize=14)
plt.legend()
...
# Run simulations
results_1 = run_T2_simulation(alpha=0.80)  # 10% increased demand
results_2 = run_T2_simulation(alpha=1.00)  # baseline
...
# Run simulations
results_1 = run_T3_simulation(alpha=0.8)  # 10% increased demand
results_2 = run_T3_simulation(alpha=1.00)  # baseline
...
# ============================================================
t1_df = pd.read_csv('data/Terminal1 Flight Data.csv')
t2_df = pd.read_csv('data/Terminal2 Flight Data.csv')
t3_df = pd.read_csv('data/Terminal3 Flight Data.csv')

# ============================================================
# 2️⃣ GET BASELINE SIMULATION-BASED WAIT TIMES
# ============================================================
# Replace with your actual simulation calls if available
t1_result = run_T1_simulation()
t2_result = run_T2_simulation()
t3_result = run_T3_simulation()
